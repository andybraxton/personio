...
function UploadZone({
  ...
  // Stop the browser downloading files dragged anywhere on the page
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {/* Drop zone */}
      {/* sr-only keeps input in DOM so it can receive focus/click — display:none breaks Safari */}
      <input
        ref={inputRef}
        id={inputId}
        type="file"
      ...
      <label
        ...
        onDrop={(e) => { e.preventDefault(); e.stopPropagation(); onChange((p) => ({ ...p, dragging: false })); onDrop(e); }}
      >
